...
# Standalone test app (avoids importing backend.main which needs catboost)
...
class TestChatEndpoint:
    ...
    def test_chat_disabled_returns_503(self):
        with patch("backend.chatbot.routes.CHATBOT_ENABLED", False):
    ...
    def test_chat_no_api_key_returns_503(self):
        with patch("backend.chatbot.routes.CHATBOT_ENABLED", True), \
             patch("backend.chatbot.routes.OPENROUTER_API_KEY", ""):
            resp = self.client.post(
                "/api/chat",
                json={"message": "hi", "session_id": "s1"},
            )
    ...
    def test_chat_invalid_request_returns_422(self):
        with patch("backend.chatbot.routes.CHATBOT_ENABLED", True), \
             patch("backend.chatbot.routes.OPENROUTER_API_KEY", "sk-test"):
    ...
    def test_chat_returns_sse_content_type(self):
        ...
        with patch("backend.chatbot.routes.CHATBOT_ENABLED", True), \
             patch("backend.chatbot.routes.OPENROUTER_API_KEY", "sk-test"), \
             patch("backend.chatbot.routes.handle_chat_message", mock_handler):
            resp = self.client.post(
                "/api/chat",
